...
// biome-ignore lint/correctness/noUnusedImports: side-effect import required for tooltip initialization
...
// biome-ignore lint/correctness/noUnusedVariables: assigned in DOMContentLoaded, used by inline event handlers
...
globalThis.loadKnowledge = async () => {
...
export const sendJsonData = async (url, data) =>
	await api.callJsonApi(url, data);
...
globalThis.pauseAgent = async (paused) => {
...
export const getChatBasedId = (id) =>
	`${context}-${globalThis.resetCounter}-${id}`;
...
// biome-ignore lint/correctness/noUnusedImports: side-effect import initializes chat navigation store
...
async function startPolling() {
	...
	async function _doPoll() {
		...
		try {
			...
			const pollOk = Boolean(result?.ok);

...
const model = {
  ...
  setTab(tab) {
    ...
    else if (tab === "teams" && this.selectedOrgId) this.loadTeams(this.selectedOrgId);
    else if (tab === "users") this.loadUsers();
    else if (tab === "groups" && this.selectedOrgId) this.loadGroupMappings(this.selectedOrgId);
  ...
  async updateOrg(orgId, data) {
    try {
      await callJsonApi("/admin_orgs", { action: "update", org_id: orgId, ...data });
      this.editingItem = null;
      await this.loadOrgs();
    } catch (e) {
      this.error = e.message;
  ...
  async deactivateOrg(orgId) {
    if (!confirm("Deactivate this organization? Members will lose access.")) return;
    try {
  ...
  async loadTeams(orgId) {
    this.loading = true;
    try {
      const res = await callJsonApi("/admin_teams", { action: "list", org_id: orgId });
      this.teams = res.data || [];
    } catch (e) {
      this.error = e.message;
    }
  ...
  async createTeam(orgId, name, slug) {
    try {
      await callJsonApi("/admin_teams", { action: "create", org_id: orgId, name, slug });
      this.showForm = false;
  ...
  async updateTeam(teamId, data) {
    try {
      await callJsonApi("/admin_teams", { action: "update", team_id: teamId, ...data });
      this.editingItem = null;
  ...
  async deactivateUser(userId) {
    ...
    try {
      await callJsonApi("/admin_users", { action: "deactivate", user_id: userId });
      await this.loadUsers();
    } catch (e) {
      this.error = e.message;
    }
  ...
  async loadGroupMappings(orgId) {
    this.loading = true;
    try {
      const res = await callJsonApi("/admin_group_mappings", { action: "list", org_id: orgId });
  ...
  async deleteGroupMapping(entraGroupId) {
    ...
    try {
      await callJsonApi("/admin_group_mappings", { action: "delete", entra_group_id: entraGroupId });
  ...
  async deleteVaultKey(vaultId) {
    ...
    try {
      await callJsonApi("/admin_api_keys", { action: "delete", vault_id: vaultId });
      await this.loadVaultKeys();
    } catch (e) {
      this.error = e.message;
    }

...
const model = {
	...
	async openSelectedProjectSkillsFolder() {
		const projectName = this.selectedProject?.name;
		if (!projectName) return;
		await browserStore.open(`$PROJECTS/${projectName}/.a0proj/skills`);
	},
	...
	async confirmCreate() {
		...
		if (this.selectedProject.git_url?.trim()) {
	...
	async confirmEdit() {
		await this.saveSelectedProject(false);
		await this.loadProjectsList();
		await modals.closeModal(editModal);
	},
	...
	getSelectedBrowsePath(...relPath) {
		return ["$PROJECTS", this.selectedProject.name, ...relPath]
			.join("/")
			.replace(/\/+/g, "/");
	},

	async browseSelected(...relPath) {
		const path = this.getSelectedBrowsePath(...relPath);

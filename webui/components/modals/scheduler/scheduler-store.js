...
// biome-ignore lint/correctness/noUnusedVariables: reserved for scheduler timing operations
const sleep = (ms = 0) =>
	new Promise((resolve) => {
...
function ensureTaskValidity(task) {
	return Boolean(task?.uuid && task.name && task.type);
}
...
function composeEditingTask(task = {}) {
	const base =
		task?.uuid ? { ...task } : { ...defaultEditingTask(), ...task };
	return {
		...base,
